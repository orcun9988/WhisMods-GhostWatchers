...
namespace ESP_Models
{
    class GallowsGhost
    {
        // - Root_M - Spine1_M - Spine2_M - Chest_M - Neck_M - Head_M - HeadEnd_M - 
        //Jaw_M - JawEnd_M - Toungle1 - Toungle2 - Toungle3 - Toungle4 - Toungle5 - Toungle6 - 
        //Scapula_R - Shoulder_R - ShoulderPart1_R - Elbow_R - ElbowPart1_R - Wrist_R - MiddleFinger1_R - MiddleFinger2_R -
        //MiddleFinger3_R - MiddleFinger4_R - ThumbFinger1_R - ThumbFinger2_R - ThumbFinger3_R - ThumbFinger4_R - IndexFinger1_R - 
        //IndexFinger2_R - IndexFinger3_R - IndexFinger4_R - RingFinger1_R - RingFinger2_R - RingFinger3_R - RingFinger4_R - PinkyFinger1_R - 
        //PinkyFinger2_R - PinkyFinger3_R - PinkyFinger4_R - Scapula_L - Shoulder_L - ShoulderPart1_L - Elbow_L - ElbowPart1_L - Wrist_L - MiddleFinger1_L - 
        //MiddleFinger2_L - MiddleFinger3_L - MiddleFinger4_L - ThumbFinger1_L - ThumbFinger2_L - ThumbFinger3_L - ThumbFinger4_L - IndexFinger1_L - 
        //IndexFinger2_L - IndexFinger3_L - IndexFinger4_L - RingFinger1_L - RingFinger2_L - RingFinger3_L - RingFinger4_L - PinkyFinger1_L - 
        //PinkyFinger2_L - PinkyFinger3_L - PinkyFinger4_L - Hip_R - HipPart1_R - Knee_R - KneePart1_R - Ankle_R - Toes_R - ToesEnd_R - Hip_L - 
        //HipPart1_L - Knee_L - KneePart1_L - Ankle_L - Toes_L - ToesEnd_L - Rope_Static - Rope2 - Rope3 - Rope4 - Rope5

        private static Vector3 Rope2, Rope3, Rope4, Rope5;
        private static Vector3 HeadEnd_M, Head_M, Neck_M, Chest_M, Spine2_M, Spine1_M, Root_M;
        private static Vector3 HipPart1_L, Knee_L, KneePart1_L, Ankle_L;
        private static Vector3 HipPart1_R, Knee_R, KneePart1_R, Ankle_R;
        ...
        public static void show_bones() //working
        {
            int gchecker = 0;
            Transform[] ghostBones = Main.Main.ghost.GetComponentInParent<Donteco.Ghost>().GetComponentInChildren<SkinnedMeshRenderer>().bones;
            foreach (Transform b in ghostBones)
            {
                if (b.name.Contains("Rope2"))
                {
                    Rope2 = Camera.main.WorldToScreenPoint(b.transform.position);
                    gchecker++;
                }
                if (b.name.Contains("Rope3"))
                {
                    Rope3 = Camera.main.WorldToScreenPoint(b.transform.position);
                    gchecker++;
                }
                if (b.name.Contains("Rope4"))
                {
                    Rope4 = Camera.main.WorldToScreenPoint(b.transform.position);
                    gchecker++;
                }
                if (b.name.Contains("Rope5"))
                {
                    Rope5 = Camera.main.WorldToScreenPoint(b.transform.position);
                    gchecker++;
                }

                if (b.name.Contains("HeadEnd_M"))
                {
                    HeadEnd_M = Camera.main.WorldToScreenPoint(b.transform.position);
                    gchecker++;
                }
            ...
            if (gchecker >= 29)
            {
                ESP.DrawBoneLine(Rope2, Rope3, Color.red);
                ESP.DrawBoneLine(Rope3, Rope4, Color.red);
                ESP.DrawBoneLine(Rope4, Rope5, Color.red);

                ESP.DrawBoneLine(HeadEnd_M, Head_M, Color.red);
                ...
                ESP.DrawBoneLine(Root_M, HipPart1_L, Color.red);
                ...
                ESP.DrawBoneLine(Root_M, HipPart1_R, Color.red);
